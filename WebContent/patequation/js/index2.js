...
var eq = new Equation(5, "x+4=10", 50);
eq.steps = ["x+4-4=10-4", "x+0=10-4", "x+0=6"];
var equations = [
...
//    new Equation("x+4=3*2", 70)];
    eq,
    //new Equation("7x=56", 30),
    new Equation(5, "x+4=3*2", 50),
    new Equation(5, "3x+7=19", 40),
    new Equation(5, "x=18-7x", 60),
    new Equation(5, "x-10=4-x", 50),
    new Equation(5, "3*(x-2)=25", 70),
    new Equation(5, "-2*(x-4)=54", 70),
    new Equation(5, "(5x)/(4)=20", 60),
    new Equation(5, "(x+2)/(3)=-9", 80),
    new Equation(5, "(x-5)/(2)=2*(x+1)", 100)];
var concluded = 0;
...
function Equation(id, equation, points) {
	this.id = id;
    ...
    this.lastStep = ""; //a string with the last step from user
    this.steps = null; //array os steps by student
...
function loadEquation(index) {
    var firstLine = $(selectedSheet + " .hLineAux").next().text();
    firstLine = firstLine.replace(/^\s+|\s+$/g, "");//trim function
    selectedEquation = equations[index];

    //alert(firstLine + " + " + selectedEquation.equation);
    if (replaceAll(firstLine, " ", "") !== replaceAll(selectedEquation.equation, " ", "")){
        $(selectedSheet + " .hLine").text("");
        firstLine = $(selectedSheet + " .hLineAux").next().text();
        firstLine = firstLine.replace(/^\s+|\s+$/g, "");//trim function
    }
...
//        if (pedir operacao){
//            "<a class='operation' data-toggle='modal' href='#boxOperations'></a>"
//        }

        if (selectedEquation.steps !== null && selectedEquation.steps.length > 0) {
            for (var i = 0; i < selectedEquation.steps.length; i++) {
                stack = textToMathml(selectedEquation.steps[i]);

                elements = "<ul>";
                for (var j = 0; j < stack.length; j++) {
                    elements = elements + "<li><math>" + stack[j] + "</math></li>";
                }
...
//            var equation = getEquation(passo);
            ...
            if (text.indexOf("a=") !== -1 && text.indexOf("b=") !== -1 && text.indexOf("c=") !== -1) {
                selectedEquation.lastStep = text;

            }
            $(selectedSheet + " .labelDefault input").attr("value", selectedEquation.lastStep);
            focus();
            //}
        }
...
//            var naturalHtml = $(this).html();
            ...
            selectedEquation.lastStep = naturalText;
...
//                }
    ...
    if (selectedEquation.lastStep.length > 0) {
        $(selectedSheet + " .canMove li input").animate({
            width: (selectedEquation.lastStep.length + 1) * 16
        }, 100, centralizeCanMoveAndButton);
    }
...
function requestServer(type, last, next, typeOperation, element) {
    ...
    $.ajax({url: "http://localhost:8080/pat2math/resolver?id=" + selectedEquation.id + "&echo=" + msg +
    	"&callback=?",
        dataType: 'jsonp',
