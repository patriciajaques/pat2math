...
a.s.buttons[j].click:-1},"boolean"==typeof a.s.buttons[j].click?(j.toLowerCase()=="não, obrigado."||"finalizar"==j.toLowerCase()||j.toLowerCase() == "fechar" ||"entendi"==j.toLowerCase()||"ok"==j.toLowerCase()||"legal"==j.toLowerCase()||"sim"==j.toLowerCase())&&k.hideAll||("próximo"==j.toLowerCase()||"só se for agora!"==j.toLowerCase()||"vamos começar!"==j.toLowerCase()||"sim, por favor."==j.toLowerCase()||"desejo recomeçar o tour."==j.toLowerCase())&&k.next||"voltar"==j.toLowerCase()&&k.prev:function(b){return~b.data.guider&&b.data.guider.call(a.s.e,b)}))}b|=p}b&J&&(a.p=(a.s.position=function(){for(var b=0;b<y.length;b++)if(y[b].toLowerCase()==a.s.position.toLowerCase())return y[b]}()||x.position).replace(/(.*)[A-Z].*/,"$1,"+a.s.position).split(","),b|=p);if(n==a.s.name&&b&(K|p)){var h,c,e,j,d=a.e.innerHeight();h=a.e.innerWidth();
