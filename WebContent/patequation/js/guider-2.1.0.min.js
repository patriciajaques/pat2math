...
a.s.buttons[j].click:-1},"boolean"==typeof a.s.buttons[j].click?(j.toLowerCase()==guiderKeysTXT[0]||guiderKeysTXT[2]==j.toLowerCase()||j.toLowerCase() == guiderKeysTXT[1] ||guiderKeysTXT[3]==j.toLowerCase()||guiderKeysTXT[4]==j.toLowerCase()||guiderKeysTXT[5]==j.toLowerCase()||guiderKeysTXT[6]==j.toLowerCase())&&k.hideAll||(guiderKeysTXT[7]==j.toLowerCase()||guiderKeysTXT[8]==j.toLowerCase()||guiderKeysTXT[9]==j.toLowerCase()||guiderKeysTXT[10]==j.toLowerCase()||guiderKeysTXT[11]==j.toLowerCase())&&k.next||guiderKeysTXT[12]==j.toLowerCase()&&k.prev:function(b){return~b.data.guider&&b.data.guider.call(a.s.e,b)}))}b|=p}b&J&&(a.p=(a.s.position=function(){for(var b=0;b<y.length;b++)if(y[b].toLowerCase()==a.s.position.toLowerCase())return y[b]}()||x.position).replace(/(.*)[A-Z].*/,"$1,"+a.s.position).split(","),b|=p);if(n==a.s.name&&b&(K|p)){var h,c,e,j,d=a.e.innerHeight();h=a.e.innerWidth();
