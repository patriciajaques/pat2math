function introduction (nextStep) {
	if (nextStep === "") {
		$.guider({
			...
			description : "<center>O PAT2Math é um programa que auxilia na resolução de equações algébricas. Vamos conferir alguns funcionamentos básicos antes de começar.</center>",
			overlay : "dark",
			width : 600,
			alignButtons : "right",
...
function clickEquation ( ) {
	$.guider({
	    ...
	    alignButtons: "right", 
	    onShow: function() {setCookieDays (cStepTour, "click", 1); setCookieDays (cFunctionTour, "clickEquation", 1);},
	    buttons: {
	    	Entendi: {
                click: true,
                className: "primary"
...
function plan2Explanation (nextStep) {
	...
	$("#tour").guider({
		...
		position: "left",
		alignButtons: "right",
		onShow: function() {setCookieDays (cStepTour, "finish", 1); setCookieDays (cFunctionTour, "plan2Explanation", 1);},
		closable: true, 
		buttons: {
			Voltar: true,
			Finalizar: {
