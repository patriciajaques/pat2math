...
// equações em sequência. Se não estiver, a variável pos deverá ser atualizada
// //quando clicar no botão da próxima equação.
// var pos = -1; //Posição da equação atual no array ordenado (currentIds2)

// OBS: o método funciona se não houverem frações dentro de frações, estudar uma
...
function calculateLength(equation) {
	...
	// Se o array de posições tiver pelo menos um registro, existe uma ou mais
	// frações na equação
	if (posFracArray.length > 0) {
		// Converte a String em um array de caracteres
		var equationArray = equation.split("");
		// Na primeira execução, first é o início da equação e last é a posição
		// do primeiro símbolo de fração
		var first = 0, last = posFracArray[0];
		// posArray recebe 1 pois a posição 0 já foi utilizada acima
		var posArray = 1;
		// O próximo laço while deverá ser executado enquanto o último símbolo
		// de fração da equação não foi verificado
		var continua = last !== undefined;

		while (continua) {
			// Controlam o balanceamento de parênteses abertos e fechados
			var cont1 = 0, cont2 = 0;
			// length1 é o tamanho do numerador e length2 do denominador da
			// fração
			var length1, length2;
			// O primeiro for começará de trás para frente, a partir do primeiro
			// caracter que não seja o símbolo de fração nem o parênteses que
			// fecha o numerador
			var i = last - 2;
			// Para os parênteses estarem balanceados, cont2 deve ser o número
			// sucessor a cont1
			for (; cont1 !== cont2 + 1; i--) {
				if (equationArray[i] === "(")
					cont1++;
				// Se este parêntese for encontrado, é que haviam parênteses
				// internos no numerador da fração, como por exemplo 5*(x+2)
				else if (equationArray[i] === ")")
					cont2++;
			}

			length1 = last - i + 1; // seria -1+2, onde 2 são os parênteses que
									// abrem e fecham a fração, os quais não
									// deverão ser contados no tamanho final
			// O -1 é porque, antes de sair do laço for, ou seja, quando a
			// condição for falsa, i é decrementado mais uma vez
			...
			if (last !== undefined)
				posArray++;
			// Se last for undefined, é que chegou ao último símbolo de fração,
			// que está na última posição do array de caracteres
			else {
			...
			for (; cont1 !== cont2 + 1; i++) {
				if (equationArray[i] === ")")
					cont1++;
				// Se este parêntese for encontrado, é que haviam parênteses
				// internos no denominador da fração, como por exemplo 5*(x+2)
				else if (equationArray[i] === "(")
					cont2++;
			}

			length2 = i - first + 2; // 2 são os parênteses que abrem e
										// fecham a fração

			// Compara qual dos tamanhos é o menor, que deverá ser subtraído do
			// tamanho total da equação (o que conta é o termo com mais
			// caracteres)
			if (length2 < length1)
				length -= length2;
...
function enableContent(id) {
	$.post(appContext + "content/enable", {
		"idContent" : id
	}, function(data) {
		$("#content-" + id).html("Ok");
	});
...
function loadTasks(id) {
	...
	if (isUnlocked) {
		...
		// }
		// else if(id === 2 || id === 10 || id === 18 || id === 26 || id ===
		// 34){
		// setBackgroundColor("#EEE8AA");
		// }
		// else if(id === 3 || id === 11 || id === 19 || id === 27 || id ===
		// 35){
		// setBackgroundColor("#E0FFFF");
		// }
		// else if(id === 4 || id === 12 || id === 20 || id === 28 || id ===
		// 36){
		...
		if (open == 'none' || open === undefined) {
					...
					.ajax({
						...
						success : function(data) {
							if (id <= numPlanosIntroducao
								...
								// equação inicial
								// já seja a resposta, por isso é realizada essa
								// manipulação
							...
							// Verificação especial se o aluno já está nos
							// planos de aula convencionais
							// if (id > numPlanosIntroducao) {
							// id -= numPlanosIntroducao;
							//					
							// if (enableWorkedExamples && id !== 6 && id !== 11
							// && id !== 16 && id !== 21 && id < 25) { //id !==
							// 25 && id !== 29 && id !== 33 && id < 36
							// $("#tasks" + id).html('<span class="taskWE"
							// onclick="getResolutionEquation(' + id + ')"
							// id="taskWE"' + id + '>' + equationsWE[id] +
							// '</span> <i style="margin-right: 6px"
							// class="icon-pencil icon-white"></i> <i
							// class="icon-ok icon-white"></i><br>' +
							// document.getElementById("tasks" + id).innerHTML);
							//
							// //Verificação especial via cookie, caso o usuário
							// já tenha visualizado o exemplo trabalhado,
							// //mas ainda não concluiu a resolução de nenhuma
							// equação
							// if ((id > 2 || enableTourInterativo === false) &&
							// taskSolved === 0 && getCookie("visualizedWE") ===
							// "") {
							// setTimeout(function()
							// {loadExerciseWE(equationsWE[id], pointsWE[id]);},
							// 1000);
							...
							if (id !== 1 && id !== 6 && id !== 11 && id !== 16
								...
								if (taskSolved === 0) {
									...
									if (cookieEquation !== "") {
										...
										// Se a String do ID do plano
										// selecionado estiver na primeira
										// posição da String do ID da equação,
										// significa que
			...
			// Easter Egg do plano final do TCC. Se o usuário clicar sobre o 42
			// após abrir o menu com as equações disponíveis, ele será
			// redirecionado para a página do Google
			// com a calculadora indicando 42 para "resposta para a vida, o
...
function getNumEquationsPlan() {
	...
	// return 4;
	//		
	// else if (planoAtual === 1002 || planoAtual === 1003 || planoAtual ===
	// 1004 || planoAtual === 1007)
	// return 5;
	//		
	...
	//	
	// else {
	if (planoAtual < 19 && planoAtual !== 0 && planoAtual !== 6
...
function getPontuacaoPlano() {
	if (levelGamification !== undefined) {
		...
		else if (planoAtual === 1012 || planoAtual === 1013
				|| planoAtual === 1014)
			pontuacaoPlano = 100;
...
function previousEquation() {
	if (idEquation > planoAtual * 100) // Verifica se o id atual é maior que o
										// id da primeira equação do plano
		loadExercise(idEquation - 1);
...
function changePlan(idGroup, idPlan) {
	$.post(appContext + "group/changePlan", {
		"idGroup" : idGroup,
		"idPlan" : idPlan
	}, function(data) {
...
function changeGroup(idGroup) {
	$.post(appContext + "student/changeGroup", {
		"idGroup" : idGroup
	}, function(data) {
		$(".item").attr("class", "item2");
